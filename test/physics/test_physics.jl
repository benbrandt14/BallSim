...
@testset "Physics: Invariants" begin
    @testset "Energy Conservation (Vacuum) - 2D" begin
        ...
        gravity = (p, v, m, t) -> SVector(0.0f0, 0.0f0) # Vacuum
    ...
    @testset "Energy Conservation (Vacuum) - 3D" begin
        ...
        sys.data.active[1] = true
        sys.data.pos[1] = SVector(0.0f0, 0.0f0, 0.0f0)
        ...
        gravity = (p, v, m, t) -> SVector(0.0f0, 0.0f0, 0.0f0) # Vacuum
    ...
    @testset "Gravity Acceleration" begin
        ...
        gravity = (p, v, m, t) -> SVector(0.0f0, -10.0f0)
        ...
        @test isapprox(sys.data.vel[1], SVector(0.0f0, -0.5f0), atol = 1e-5)

        # 5. Check Position
        # p_new = p + v_new * dt = (0, 0) + (0, -0.5) * 0.1 = (0, -0.05)
        # Note: step! does v_new = v + a*dt, then p_new = p + v_new * dt (Semi-Implicit Euler)
        @test isapprox(sys.data.pos[1], SVector(0.0f0, -0.05f0), atol = 1e-5)
    end
    ...
    @testset "Tunneling Prevention (Thick Wall)" begin
        ...
        sys = Common.BallSystem(1, 2, Float32)
        sys.data.active[1] = true
        sys.data.pos[1] = SVector(0.9f0, 0.0f0)
        sys.data.vel[1] = SVector(100.0f0, 0.0f0) # Very fast
        ...
        dist = Common.sdf(boundary, sys.data.pos[1], 0.0f0)
        @test dist <= 1e-4
    ...
    @testset "Tunneling Prevention (Thin Box)" begin
        ...
        sys = Common.BallSystem(1, 2, Float32)
        sys.data.active[1] = true
        sys.data.pos[1] = SVector(4.9f0, 0.0f0)
        sys.data.vel[1] = SVector(100.0f0, 0.0f0)
    ...
    @testset "Collision Counting - 2D" begin
        ...
        sys.data.vel[1] = SVector(1.0f0, 0.0f0) # Moving towards right wall at x=1
    ...
    @testset "Collision Counting - 3D" begin
        ...
        gravity = (p, v, m, t) -> SVector(0.0f0, 0.0f0, 0.0f0)
    ...
    @testset "Inverted Circle (Obstacle)" begin
        ...
        sys = Common.BallSystem(1, 2, Float32)
        sys.data.active[1] = true
        sys.data.pos[1] = SVector(1.05f0, 0.0f0)
    ...
    @testset "Ellipsoid Reflection" begin
        ...
        sys.data.active[1] = true
        sys.data.pos[1] = SVector(4.9f0, 0.0f0)
        sys.data.vel[1] = SVector(5.0f0, 0.0f0)
    ...
    @testset "Restitution (Energy Loss)" begin
        ...
        sys = Common.BallSystem(1, 2, Float32)
        sys.data.active[1] = true
        sys.data.pos[1] = SVector(4.9f0, 0.0f0) # Near right wall
        sys.data.vel[1] = SVector(10.0f0, 0.0f0) # Moving right
