...
@testset "Extended Shapes" begin
    ...
    @testset "Inverted Wrapper" begin
        ...
        @test Common.sdf(circle, p_inside, 0.0f0) < 0
    ...
    @testset "Ellipsoid" begin
        ...
        @test isapprox(Common.sdf(ell, p_surf_x, 0.0f0), 0.0f0, atol = 1e-4)

        # Test Point on Surface (Y-axis)
        p_surf_y = SVector(0.0f0, 5.0f0)
        @test isapprox(Common.sdf(ell, p_surf_y, 0.0f0), 0.0f0, atol = 1e-4)

        # Test Point Outside (X-axis)
        p_out = SVector(12.0f0, 0.0f0)
        @test isapprox(Common.sdf(ell, p_out, 0.0f0), 2.0f0, atol = 1e-4)
    end
end
