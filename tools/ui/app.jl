...
function generate_preview(model)
    ...
    if g_type == "Uniform"
        arrows!(
            ...
            [0.0],
            [0.0],
            [model.gravity_vector_x[]],
            [model.gravity_vector_y[]],
            lengthscale = 0.1,
        )
    ...
    # Limits
    lim =
        max(model.boundary_radius[], model.boundary_width[], model.boundary_height[]) * 1.2
    if lim == 0
        lim = 1.0
    end
...
function build_config(model)
    ...
    if g_type == "Uniform"
        grav_params[:vector] =
            model.dimensions[] == 2 ? [model.gravity_vector_x[], model.gravity_vector_y[]] :
        ...
        [
            ...
            row([
                cell([
                    btn(
                        "Refresh Preview",
                        @click("refresh_preview_btn = true"),
                        color = "primary",
                    ),
                    br(),
                    imageview(
