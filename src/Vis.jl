...
function compute_density!(grid::Matrix{Float32}, sys::Common.BallSystem{2, T}, limit::Float64) where T
    ...
    # Iterate over the BitVector mask efficiently
    # StructArrays allows iteration over just the active ones if we wanted, 
    # but simple loop with check is fine for now.
